...
| Stale decision replay | One immutable approval operation/idempotency key, a random one-time process-local capability, operator-role enforcement, and terminal consumption | Provider authentication and persisted version-conflict proof remain proposed |
