...
The domain layer contains typed Zod contracts and canonical SHA-256 serialization. It imports no UI, database, provider, or AI library. The application layer orchestrates the evidence, opportunity, experiment, and control receipt. The UI calls one sandbox route and records human approval only in client memory.
...
Each detector declares one requirement ID, issue code, rule version, and required fact set. Evaluation is a pure function:

1. Parse the fixture schema.
2. Resolve the exact detector.
3. Return `INDETERMINATE / DETECTOR_UNAVAILABLE` when missing.
4. Return `REJECT` plus the requirement-specific issue code when any required fact is absent.
5. Return `PASS` only when every required fact is true.
6. Canonically order the receipt and calculate SHA-256.
...
- `POST /api/workspace/run` — runs the repository-owned synthetic vertical and returns `DecisionRailWorkspaceRun.v1`. Response header `X-DecisionRail-Mode` states `synthetic-no-external-mutation`.

Proposed, not implemented:

- authenticated customer-evidence intake;
- opportunity persistence/versioning;
- experiment execution queue;
- human product-decision persistence;
- immutable handoff bundle export.

## Persistence and RLS

`supabase/migrations/001_decision_rail.sql` defines eight tenant-scoped tables. Every table enables and forces RLS. Policies require `auth.uid()` membership and role checks. `product_decisions` enforces `builder_id <> approver_id`; update and delete are revoked for decisions and receipts.
