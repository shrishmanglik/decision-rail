...
describe("human authority gate", () => {
  ...
  it("requires a non-builder receiving operator for accepted handoff", () => {
    const base = { builderId: "builder-local-demo", approvalReceipt: `DEMO-APPROVAL:reviewer-local:${"a".repeat(16)}`, controlDigest: digest };
    ...
    expect(createDemoHandoffReceipt({ ...base, operatorId: "operator-local" }))
      .toEqual({ ok: true, receipt: `DEMO-HANDOFF:ACCEPTED:operator-local:${"a".repeat(16)}` });
  });
});
